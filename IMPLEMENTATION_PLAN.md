...
## 🚀 个性化推荐系统总结 (Stage B Finish)

### 1. 功能描述
- **实时兴趣感知**：系统能够毫秒级捕捉用户在短视频中的互动行为（点赞、收藏、评论、中长视频完播），并实时修正用户的兴趣画像。
- **智能化 Feed 流**：推荐列表不再是简单的倒序列表，而是根据用户的“标签偏好”量身打造的。比如你偏爱“生活”，你的列表中将有 70% 的内容属于生活类。
- **精准去重机制**：系统弃用了粗暴的批量过滤，实现了基于**真实起播（WATCH_START）**的精准去重。只有用户真正滑到并观看的视频才会被标记为已看，确保优质内容不被浪费。
- **无限下拉循环**：当用户刷完全平台所有新视频后，系统会自动重置刷过记录，实现如同抖音、快手般的无缝循环消费体验。

### 2. 简要实现方式
- **行为异步链路 (Kafka)**:
  - 在 `video-interaction` 微服务中埋点，通过 Kafka 削峰填谷，将高频的点赞/播放/起播事件异步发送至 `user-behavior-topic`。
  - 消费端（`video-video`）根据视频 ID 自动反向解析其包含的标签，实现“行为到标签”的转化。
- **实时画像工厂 (Redis)**:
  - 使用 Redis Hash (`user:profile:{userId}`) 存储标签权重。利用 `HINCRBYFLOAT` 原子操作累加分值。
  - 设置 30 天滑动过期，既保证了画像的灵敏度，又清理了僵尸数据。
- **三级瀑布式召回 (Recurrent Retrieval)**:
  - **一级召回**：从 Redis 获取 Top-3 标签，到数据库中进行精准标签匹配（占 70% 权重）。
  - **二级召回**：全局热度召回（高播放量），确保爆款内容能够分发给每一位用户。
  - **三级召回**：新鲜度召回（最新发布），实现冷启动内容的曝光。
- **精准去重与重置**:
  - **标记分离**：`getVideoFeed` 接口仅负责下发内容，不主动标记已看。
  - **行为触发**：由 Kafka 消费者在接收到 `WATCH_START` 信号后，动态维护 Redis Set (`user:seen:{userId}`)。
  - **原子重置**：在查询 SQL 中通过 `NOT IN` 避开已看 ID，若判定结果集 < 3 条时自动 `DEL` 浏览记录以开启新循环。
